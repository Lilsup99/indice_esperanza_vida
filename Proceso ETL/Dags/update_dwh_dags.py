...
internet_path = 'gs://pf_digital_code_storage/internet.csv'

def transform_data(data_path, internet_path, data_output_path):
    ...
    # Integrar al Dataframe los datos de internet
    df2 = pd.read_csv(internet_path)
    df2 = df2.rename(columns={'Code':'country', 'Year':'year'})
    df2 = df2.drop(columns=['Unnamed: 0','Entity','Cellular Subscription','No. of Internet Users', 'Broadband Subscription'])

    df = df.merge(df2, how='left', on=['country', 'year'])
